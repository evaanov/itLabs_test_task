<template>
    <div class="popup" v-if="show">
        <div class="popup_content">
            <slot>
                <div class="add_user">
                    <form @submit.prevent>
                        <div class="input">
                        <p>ФИО</p>
                        <input type="text" v-model="post.name">
                        </div>
                        <div class="input">
                        <p>Компания</p>
                        <input type="text" v-model="post.company">
                        </div>
                        <div class="input">
                        <p>Группа</p>
                        <select v-model="post.group">
                            <option disabled value="">Выбрать</option>
                            <option>Клиент</option>
                            <option>Прохожий</option>
                            <option>Партнер</option>
                        </select>
                        </div>
                        <div class="input">
                        <p>Присутствие</p>
                        <input  class="checkbox" type="checkbox" v-model="post.isHere">
                        </div>
                        <div class="btns">
                        <button class="add_btn" @click="createUser" v-if="this.post.name === '' || this.post.company === '' || this.post.group === ''" disabled>Добавить</button>
                        <button class="add_btn" @click="createUser" v-else>Добавить</button>
                        <button class="close_btn" @click="hidePopup">Закрыть</button>
                        </div>
                    </form>
                </div>
                <div class="close_svg">
                    <svg @click="hidePopup" width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M23 0C18.451 0 14.0042 1.34893 10.2219 3.8762C6.43956 6.40347 3.4916 9.99558 1.75078 14.1983C0.00996558 18.401 -0.445511 23.0255 0.441949 27.4871C1.32941 31.9486 3.51995 36.0468 6.73655 39.2634C9.95316 42.4801 14.0514 44.6706 18.5129 45.5581C22.9745 46.4455 27.599 45.99 31.8017 44.2492C36.0044 42.5084 39.5965 39.5604 42.1238 35.7781C44.6511 31.9958 46 27.549 46 23C46 19.9796 45.4051 16.9888 44.2492 14.1983C43.0934 11.4078 41.3992 8.87229 39.2635 6.73654C37.1277 4.6008 34.5922 2.90663 31.8017 1.75077C29.0112 0.594913 26.0204 0 23 0ZM28.8267 31.326L23 25.4993L17.1733 31.326C17.0092 31.4901 16.8144 31.6203 16.6 31.7091C16.3856 31.7979 16.1558 31.8436 15.9237 31.8436C15.6916 31.8436 15.4618 31.7979 15.2474 31.7091C15.0329 31.6203 14.8381 31.4901 14.674 31.326C14.5099 31.1619 14.3797 30.9671 14.2909 30.7526C14.2021 30.5382 14.1564 30.3084 14.1564 30.0763C14.1564 29.8442 14.2021 29.6144 14.2909 29.4C14.3797 29.1856 14.5099 28.9908 14.674 28.8267L20.5007 23L14.674 17.1733C14.3426 16.8419 14.1564 16.3924 14.1564 15.9237C14.1564 15.4549 14.3426 15.0054 14.674 14.674C15.0054 14.3426 15.455 14.1564 15.9237 14.1564C16.3924 14.1564 16.8419 14.3426 17.1733 14.674L23 20.5007L28.8267 14.674C28.9908 14.5099 29.1856 14.3797 29.4 14.2909C29.6144 14.2021 29.8443 14.1564 30.0763 14.1564C30.3084 14.1564 30.5382 14.2021 30.7527 14.2909C30.9671 14.3797 31.1619 14.5099 31.326 14.674C31.4901 14.8381 31.6203 15.0329 31.7091 15.2473C31.7979 15.4618 31.8436 15.6916 31.8436 15.9237C31.8436 16.1557 31.7979 16.3856 31.7091 16.6C31.6203 16.8144 31.4901 17.0092 31.326 17.1733L25.4993 23L31.326 28.8267C31.6554 29.1592 31.8402 29.6083 31.8402 30.0763C31.8402 30.5444 31.6554 30.9935 31.326 31.326C31.1628 31.4917 30.9683 31.6233 30.7537 31.7131C30.5392 31.8029 30.3089 31.8491 30.0763 31.8491C29.8438 31.8491 29.6135 31.8029 29.399 31.7131C29.1844 31.6233 28.9899 31.4917 28.8267 31.326Z" fill="#666666"/>
                    </svg>
                </div>
            </slot>
        </div>
    </div>
</template>

<script>
export default {
    name: "add_user",
    props: { 
        show: { 
            type: Boolean,
            default: false
        },
        users: { 
            type: Array,
            required: true
        }
    },
    data () { 
      return { 
        post: { 
        name: '',
        company: '',
        isHere: null,
        group: '',
        }
      }
    },
    methods: { 
        hidePopup() { 
            this.$emit("update:show", false)
        },
        createUser() { 
        this.post.id = this.users.length + 1;
        this.$emit('create', this.post)
        // this.users.push(this.post);
        this.post = {
          name: '',
          company: '',
          isHere: null,
          group: '',
        }
      }
    }
}
</script>

<style>
.popup { 
    top: 0;
    bottom: 0;
    right: 0;
    left: 0;
    background: rgb(0, 0, 0, 0.4);
    position: fixed;
    display: flex;
}

.popup_content { 
    margin: auto;
    background-color: #fff;
    border-radius: 12px;
    width: 1461px;
    height: 581px;
    padding: 20px;
    display: flex;
    flex-direction: row;
}

.add_user { 
    width: 775px;
    height: 475px;
    margin: auto;
    margin-bottom: 40px;
}

.add_user form { 
    height: 100%;
}

.input { 
    display: flex;
    flex-direction: row;
    gap: 83px;
    height: 52px;
    margin-bottom: 40px;
    font-size: 32px;
    line-height: 43.58px;
}

.input p { 
    width: 190px;
    margin: 0;
    color: #4E3000;
}

.input input { 
    width: 502px;
    border: none;
    box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.09);
    border-radius: 7px;
}

.input select {
    width: 502px;
    padding: 0px 31px;
    border: none;
    box-shadow: 0px 0px 20px 0px rgba(0, 0, 0, 0.09);
    border-radius: 7px;
}

.input .checkbox { 
    width: 34px;
    height: 34px;
}

.btns { 
    gap: 34px;
    width: 580px;
    height: 52px;
    display: flex;
    flex-direction: row;
}

.btns .add_btn { 
    width: 273px;
    height: 52px;
    border: none;
    background-color: #4CAF50;
    color: #fff;
    font-size: 24px;
    line-height: 28.13px;
    border-radius: 7px;
}

.btns .close_btn { 
    width: 273px;
    height: 52px;
    border: none;
    background-color: #737373;
    color: #fff;
    font-size: 24px;
    line-height: 28.13px;
    border-radius: 7px;
}
</style>